class Snippet < ApplicationRecord
  ...
  scope :published, -> { where('date_modified <= ?', Date.today) }
  scope :scheduled, -> { where('date_modified > ?', Date.today) }
  ...
  # UNUSED?
  # def file_slug
  #   file_name.slice(0..-4).to_seo_slug
  # end
